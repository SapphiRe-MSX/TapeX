...
It currently supports CAS, TAP and TZX files, providing a common block-based representation while preserving format-specific information. TapeX can also identify some of the semantic content stored in tape blocks, including MSX and ZX Spectrum headers.
...
* Automatic detection of CAS, TAP and TZX tape images.
...
Then create a TapeX instance from an `ArrayBuffer` or a `Uint8Array`:
...
Returns the TZX format version.
...
Returns the TZX file header. For CAS and TAP images, returns an empty `Uint8Array`.
...
Each block is initially displayed in a compact form and can be expanded to show its detailed information. Data fields provide a preview of their contents that can be toggled between preview and full views.
